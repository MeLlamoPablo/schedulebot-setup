...
/**
 * Sends two requests to the Heroku API using the provided token to shut down the web process
 * and start the bot process.
 *
 * @param {string} herokuAppName
 * @param {string} herokuApiKey
 * @returns {Promise<void>}
 */
function turnOnBot(herokuAppName, herokuApiKey) {
	return new Promise((fulfill, reject) => {

		const client = new Heroku({ token: herokuApiKey });

		client.patch(`/apps/${herokuAppName}/formation/web`, {
			quantity: 0
		})
			.then(res => {
				console.log(`To the /apps/${herokuAppName}/formation/web with body`);
				console.log({quantity: 0});
				console.log("Heroku responded with:");
				console.log(res);
			})
			.then(() => client.patch(`/apps/${herokuAppName}/formation/bot`, {
				quantity: 1
			}))
			.then(res => {
				console.log(`To the /apps/${herokuAppName}/formation/bot with body`);
